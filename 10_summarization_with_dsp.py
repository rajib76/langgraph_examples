...
system_prompt = """
You are the policy model as mentioned in the paper 
### Guiding Large Language Models via directional Stimulus Prompting ###. 
Your job is to create hints from a provided chat transcript that can be 
provided to the LLM which is responsible to create an email summary 
that will capture the action items 
of all the participants in the meeting.
"""
user_prompt = """
...
Extract the key discussion items and action items from the provided chat transcript between 
a bank customer and a loan agent. 
Focus on identifying tasks for both the bank customer and a loan agent, 
such as:
{{- '\n' -}}
{%- for hint in hints -%}
    {{hint.hint}},
{% endfor %}
After extracting the relevant content, please draft an email that can be sent to the bank customer. 
Please only share the draft email, DO NOT ADD anything else.
"""
...
def generate_hint_v2(state):
    ...
    hints = completion.choices[0].message.parsed.hints

    return {"messages": [hints]}


def generate_summary(state):
    hints = state["messages"][-1]
    template = Template(summary_system_prompt)
    rendered_template = template.render(hints=hints)
    print("Prompt with Hints ", rendered_template)
    completion = client.beta.chat.completions.parse(
        model="gpt-4o-2024-08-06",
        messages=[
            {"role": "system",
...
graph_builder.add_node("generate_hint", generate_hint_v2)
graph_builder.add_node("generate_summary", generate_summary)
graph_builder.set_entry_point("generate_hint")
graph_builder.add_edge("generate_hint", "generate_summary")
